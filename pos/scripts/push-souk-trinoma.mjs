...
const EVENT_NAME = 'Souk Trinoma (Matcha + Ube)';
const EVENT_KEY = 'popup-souk-trinoma-matcha-ube';
const ARCHIVE_KEY_PREFIXES = [
  'popup-manila-matcha-fest-2026'
];
const menu = JSON.parse(
  fs.readFileSync(path.join(__dirname, '..', 'data', 'souk_trinoma_menu.json'), 'utf8')
);
...
if (archivedNames.length) {
  console.log(`ARCHIVED ${archivedNames.length} Manila Matcha Fest event(s):`);
  for (const n of archivedNames) console.log(`  - ${n}`);
} else {
  console.log('No active Manila Matcha Fest events to archive.');
}
