...
(function (global) {
  ...
  /** Palm width in normalized space — scales pinch threshold per hand. */
  ...
  /**
   * Map normalized landmark to a 3D world point (Three.js style: Y up, Z toward viewer when closer).
   * MediaPipe z is negative when closer to the camera.
   */
  ...
  /** Which side of the mirrored preview the hand is on (user's right = screen right). */
  function isScreenRightSide(hand, mirror) {
    return mirrorX(hand[0].x, mirror) > 0.5;
  }

  /**
   * Tracks how large the hand appears in frame (palm width).
   * Small palm = far from camera, large palm = close — used for real 3D Z placement.
   */
  function createDepthCalibrator() {
    ...
    return {
      ...
      /** @returns {number} 0 = far (small in frame), 1 = near (large in frame) */
  ...
  /**
   * Pinch aligned to video overlay. Z depth comes from hand size in frame (not just stroke width).
   * @param {object} [depthOpts]
   * @param {ReturnType<typeof createDepthCalibrator>} depthOpts.calibrator
   * @param {number} [depthOpts.zFar] world Z when hand appears small / far
   * @param {number} [depthOpts.zNear] world Z when hand appears large / close
   */
   ...
   * @param {(results: object) => void} opts.onResults
   * @param {number} [opts.maxHands]
   */
